...
int main(int argc, char *argv[]) {
	...
	f0[0] = (double*)malloc(N * N * sizeof(double));
	f1[0] = (double*)malloc(N * N * sizeof(double));
	fend[0] = (double*)malloc(N * N * sizeof(double));
		...
		if (my_rank!=0) {
		  for(i=0; i<N; i++){
	        for(j=local_N; j<N-1; j++){
			  ...
			  fend[i][j] = 0.01*(leftNeighbor[j]+f1[i+1][j]+f1[i][j-1]+f1[i][j+1]-4*f1[i][j])+2*f1[i][j]-f0[i][j];
			}
		  }
		}
		...
		if (my_rank!=comm_sz-1) {
		  for(i=0; i<N; i++){
		    for(j=0; j<local_N-1; j++){
			  ...
			  fend[i][j] = 0.01*(f1[i-1][j]+rightNeighbor[j]+f1[i][j-1]+f1[i][j+1]-4*f1[i][j])+2*f1[i][j]-f0[i][j];
			 }
		   }
		}
